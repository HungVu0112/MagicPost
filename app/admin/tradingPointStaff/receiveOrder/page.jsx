...
export default function SendOrder() {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            const res = await fetch(`/api/order/getReceiveOrder/${account.location}`, {
                method: 'GET'
            })
    ...
    return (
        <div className="xl:w-[70%] w-[95%] mr-auto ml-auto shadow-lg p-10 kanit">
            <div className="flex gap-2 text-[#FF5B00]">
                ...
                <h1 className="font-bold kanit text-2xl">Đơn hàng gửi đến</h1>
            </div>
            <div className="mt-6">
                {orders?.length !== 0 ? orders?.map(order => {
                    return <Order key={order.orderID} order={order} />
                }) : (
                    <p className="font-bold text-center text-[#FF5B00] mt-4">Hiện không có đơn hàng được chuyển đến !</p>
                )}
            </div>
        </div>
    )
